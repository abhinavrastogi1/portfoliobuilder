...
import { supabase } from "@/components/config/supabase-db-config"
...
export async  function saveCurrentUser(userDetails:any){
    try {
        const {data,error}=await supabase.from("userprofiles").insert([userDetails])
        if(error){
...
try {
     ...
     const {data,error}=await supabase.from("userprofiles").select("*").eq("clerk_user_id",clerkuser?.id)
     if(error){
